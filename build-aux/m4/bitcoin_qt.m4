...
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$bitcoin_cv_static_qt" = xyes; then
    ...
    if test "x$TARGET_OS" = xwindows; then
      _BITCOIN_QT_CHECK_STATIC_PLUGINS([Q_IMPORT_PLUGIN(QWindowsIntegrationPlugin)],[-lqwindows -lQt5FontDatabaseSupport -lQt5EventDispatcherSupport -lQt5ThemeSupport -lQt5FbSupport -lQt5AccessibilitySupport -lQt5EdidSupport -lQt5WindowsUIAutomationSupport -lQt5Gui -lQt5Core -lqtpcre2 -lqtlibpng -lqtharfbuzz -lz -limm32 -lnetapi32 -luserenv -lmpr -lwtsapi32 -ldwmapi -luxtheme -lversion])
      ...
      AX_CHECK_LINK_FLAG([-framework CoreVideo], [QT_LIBS="$QT_LIBS -framework CoreVideo"], [AC_MSG_ERROR(could not link against CoreVideo framework)])
      AX_CHECK_LINK_FLAG([[-framework IOKit]],[QT_LIBS="$QT_LIBS -framework IOKit"],[AC_MSG_ERROR(could not iokit framework)])
      bitcoin_darwin_os_version_stub="$srcdir/src/qt/darwin/os_version_check_stub.c"
      bitcoin_darwin_os_version_stub_o=
      AC_MSG_CHECKING([for ___isOSVersionAtLeast])
      if test -f "$bitcoin_darwin_os_version_stub"; then
        if $CC -c "$bitcoin_darwin_os_version_stub" -o conftest_os_version_stub.o 2>/dev/null; then
          bitcoin_darwin_os_version_stub_o="$(pwd)/conftest_os_version_stub.o"
          QT_LIBS="$QT_LIBS $bitcoin_darwin_os_version_stub_o"
          AC_MSG_RESULT([using stub])
        else
          AC_MSG_RESULT([failed to compile stub])
        fi
      else
        AC_MSG_RESULT([missing stub source])
      fi
      _BITCOIN_QT_CHECK_STATIC_PLUGINS([Q_IMPORT_PLUGIN(QCocoaIntegrationPlugin)],[-lqcocoa])
      _BITCOIN_QT_CHECK_STATIC_PLUGINS([Q_IMPORT_PLUGIN(QMacStylePlugin)],[-lqmacstyle])
      if test -n "$bitcoin_darwin_os_version_stub_o"; then
        QT_LIBS="${QT_LIBS% $bitcoin_darwin_os_version_stub_o}"
      fi
...
AC_DEFUN([_BITCOIN_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
      ...
      if test "x$TARGET_OS" = xlinux; then
        ...
        QT_LIBS="$QT_LIBS -lwtsapi32 -ldwmapi -luxtheme -lversion"
      fi
    fi
])
