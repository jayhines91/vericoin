...
$(package)_conf_env+=ac_cv_lib_brotlidec_BrotliDecoderCreateInstance=no
$(package)_config_opts=--disable-shared --with-ca-fallback --without-brotli
...
define $(package)_postprocess_cmds
  if [ -f lib/pkgconfig/libcurl.pc ]; then \
    sed -i 's/ -lbrotlidec//g; s/brotli //g' lib/pkgconfig/libcurl.pc; \
  fi && \
  if nm -u lib/libcurl.a 2>/dev/null | grep -qi brotli; then \
    echo "ERROR: libcurl.a still references brotli; rebuild with --without-brotli" >&2; \
    exit 1; \
  fi
endef
