...
void PeerLogicValidation::FinalizeNode(NodeId nodeid, bool& fUpdateConnectionTime) {
    ...
    CNodeState *state = State(nodeid);
    if (state == nullptr) {
        // Can happen if DisconnectNodes races (e.g. bootstrap pauses P2P while
        // ThreadSocketHandler is also disconnecting peers).
        LogPrint(BCLog::NET, "FinalizeNode: missing nodestate for peer=%d\n", nodeid);
        return;
    }
...
bool static ProcessHeadersMessage(CNode* pfrom, CConnman* connman, CTxMemPool& mempool, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool via_compact_block)
{
...

    {
        ...
        if (nodestate == nullptr) {
            // Peer disconnected while headers were processed (e.g. bootstrap pause).
            return true;
        }
