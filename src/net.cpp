...
void CConnman::DisconnectNodes()
{
    ...
    {
        // Delete disconnected nodes (must stay under cs_vNodes so concurrent
        // DisconnectNodes callers cannot finalize the same peer twice).
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    for (const std::string& seed : seeds) {
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(seed, vIPs, nMaxIPs, true)) {
                LogPrintf("Loading addresses from DNS seed %s\n", seed);
                addSeedIPs(vIPs);
            } else if (LookupHost(host, vIPs, nMaxIPs, true)) {
