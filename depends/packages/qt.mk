...
$(package)_config_env += PKG_CONFIG_SYSROOT_DIR=/
...
define $(package)_extract_cmds
  ...
  rm -rf qtbase qttranslations qttools && \
  mkdir qtbase && \
...
define $(package)_stage_cmds
  ...
  mkdir -p $($(package)_staging_prefix_dir)/native/bin && \
  cp -f qtbase/bin/qmake qtbase/bin/moc qtbase/bin/uic qtbase/bin/rcc $($(package)_staging_prefix_dir)/native/bin/ && \
  cp -r qtbase/mkspecs $($(package)_staging_prefix_dir)/native/
endef

define $(package)_postprocess_cmds
  sed -i 's/^Version: .*/Version: $($(package)_version)/' lib/pkgconfig/Qt5*.pc && \
