...
/** Bit flags for which window edge/corner is under the cursor. */
int resizeEdgeAt(const QWidget* widget, const QPoint& pos)
{
    if (!widget) {
        return 0;
    }
    const int margin = kFramelessResizeMargin;
    int edge = 0;
    if (pos.x() <= margin) {
        edge |= 1; // left
    }
...
QSize clampWindowSize(const QSize& size, const QWidget* widget)
{
    const QSize minimum = widget->minimumSize();
    QSize clamped(qMax(size.width(), minimum.width()), qMax(size.height(), minimum.height()));
    if (QScreen* screen = widget->screen()) {
        const QRect avail = screen->availableGeometry();
        clamped.setWidth(qMin(clamped.width(), avail.width()));
        clamped.setHeight(qMin(clamped.height(), avail.height()));
    }
    return clamped;
}
} // namespace
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        walletFrame = new WalletFrame(m_node, _platformStyle, this);
    ...
    // Allow resize from the status-bar grip on platforms without edge hit-testing.
    statusBar()->setSizeGripEnabled(true);

    // Status bar notification icons
    QFrame *frameBlocks = new QFrame();
    frameBlocks->setObjectName("statusBarTray");
    ...
    modalOverlay = new ModalOverlay(enableWallet, this->centralWidget()->layout()->itemAt(1)->widget());
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a %1 address").arg(GUIUtil::GetCoinName()));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and %1: URIs)").arg(GUIUtil::GetCoinName().toLower()));
    ...
    aboutAction = new QAction(tr("&About %1").arg(GUIUtil::GetCoinName()), this);
    aboutAction->setStatusTip(tr("Show information about %1").arg(GUIUtil::GetCoinName()));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for %1").arg(GUIUtil::GetCoinName()));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your %1 addresses to prove you own them").arg(GUIUtil::GetCoinName()));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified %1 addresses").arg(GUIUtil::GetCoinName()));
    ...
    openAction->setStatusTip(tr("Open a %1: URI or payment request").arg(GUIUtil::GetCoinName().toLower()));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible %1 command-line options").arg(GUIUtil::GetCoinName()));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        tabBar->setFixedHeight(static_cast<int>(height() * 0.80f));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    if (QSystemTrayIcon::isSystemTrayAvailable()) {
        ...
        QString toolTip = tr("%1 client").arg(GUIUtil::GetCoinName()) + " " + m_network_style->getTitleAddText();
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to %1 network", "", count).arg(GUIUtil::GetCoinName()) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
// Disabling macOS App Nap on initial sync, disk and reindex operations.
    ...
    else
    {
        ...
        const QString statusText = progressBarLabel->text();
        progressBarLabel->setText(tr("%1 · %2 behind").arg(statusText, timeBehindText));
...
void BitcoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret)
{
    ...
    QString strTitle{GUIUtil::GetCoinName()};
...
void BitcoinGUI::updateSidebarLayout()
{
    if (appToolBar) {
        appToolBar->setFixedHeight(height());
    }
    if (appTabBar) {
        appTabBar->setFixedHeight(static_cast<int>(height() * 0.80));
    }
}
...
bool BitcoinGUI::nativeEvent(const QByteArray& eventType, void* message, long* result)
{
    if (eventType == "windows_generic_MSG" || eventType == "windows_dispatcher_MSG") {
        ...
        if (msg->message == WM_NCHITTEST) {
            const LONG border = kFramelessResizeMargin;
            RECT winRect;
            ...
            if (resizeWidth && x >= winRect.left && x < winRect.left + border) {
                if (resizeHeight && y >= winRect.top && y < winRect.top + border) {
                    *result = HTTOPLEFT;
                    return true;
                }
                if (resizeHeight && y < winRect.bottom && y >= winRect.bottom - border) {
                    *result = HTBOTTOMLEFT;
                    return true;
                }
                *result = HTLEFT;
            ...
            if (resizeWidth && x < winRect.right && x >= winRect.right - border) {
                if (resizeHeight && y >= winRect.top && y < winRect.top + border) {
                    *result = HTTOPRIGHT;
                    return true;
                }
...
bool BitcoinGUI::event(QEvent* event)
{
#ifndef Q_OS_WIN
    switch (event->type()) {
    case QEvent::MouseButtonPress: {
        ...
        if (mouseEvent->button() == Qt::LeftButton) {
            m_resizeEdge = resizeEdgeAt(this, mouseEvent->pos());
            if (m_resizeEdge != 0) {
                m_resizing = true;
                m_resizeMousePos = mouseEvent->globalPos();
    ...
    case QEvent::MouseMove: {
        ...
        if (m_resizing && (mouseEvent->buttons() & Qt::LeftButton)) {
            const QPoint delta = mouseEvent->globalPos() - m_resizeMousePos;
            ...
            const QSize size = clampWindowSize(geom.size(), this);
            if (m_resizeEdge & 1) {
                geom.setLeft(geom.right() - size.width() + 1);
            } else {
                geom.setWidth(size.width());
            }
            if (m_resizeEdge & 4) {
                geom.setTop(geom.bottom() - size.height() + 1);
            } else {
        ...
        const int edge = resizeEdgeAt(this, mouseEvent->pos());
        if (edge == 5 || edge == 10) {
...
void BitcoinGUI::setEncryptionStatus(int status, bool unlockStakingOnly)
{
    switch(status)
    {
    ...
    case WalletModel::Unlocked:
        ...
        // XXX: wallet interface for staking wallet unlock
...
void BitcoinGUI::updateWalletStatus()
{
    ...
    setEncryptionStatus(walletModel->getEncryptionStatus(), walletModel->wallet().isUnlockStakingOnly());
