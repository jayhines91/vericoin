...
docker run --rm \
  ...
  bash -c "
    ...
    chmod +x /build/Build-Scripts/package-linux64-tarball.sh /build/Build-Scripts/verify-static-linux.sh
    source /build/Build-Scripts/verify-static-linux.sh
    for bin in vericoind vericoin-cli vericoin-tx vericoin-wallet vericoin-qt; do
      verify_static_linux_binary /build/${OUT_DIR}/\${bin}
    done
    BINARY_DIR=/build/${OUT_DIR} OUT_DIR=/build/${OUT_DIR} \
      /build/Build-Scripts/package-linux64-tarball.sh /build
